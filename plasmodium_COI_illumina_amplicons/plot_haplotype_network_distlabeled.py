...
Example:
python /mnt/storage11/sophie/gitrepos/plasmodium_falciparum/plasmodium_cape_verde_diversity_illumina_amplicons/plot_haplotype_network_distlabeled.py \
  ...
  --group Year \
  --marker AMA1 \
  --out-prefix presence_lengths_ama1_network_by_year \
  --presence \
  --save-edges
...
def load_counts_table(counts_path: str) -> pd.DataFrame:
    ...
    required = {"SampleID", "Specimen", "Haplotype", "Reads"}
    ...
    df = df[df["Reads"] > 0]
    return df[["SampleID", "Specimen", "Haplotype", "Reads"]]
...
def pie_fracs_for_nodes(counts: pd.DataFrame, hap_ids: List[str], group_col: str, presence: bool = False
    ...
    if presence:
        # Count the unique specimens per haplotype per group (eg. Year)
        agg = (counts.drop_duplicates(["Haplotype", "Specimen"])
                      .groupby(["Haplotype", group_col])["Specimen"]
                      .nunique()
                      .reset_index()
                      .rename(columns={"Specimen": "value"}))
    else:
